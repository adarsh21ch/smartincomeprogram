...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // make the client wait for the Gmail round-trip (~1-2s).
    if (page.send_confirmation_email && email) {
      const sendEmailInBackground = async () => {
        try {
          ...
          const emailBody = (page.email_body || '').replace(/\{\{name\}\}/g, name || 'there')
            .replace(/\{\{email\}\}/g, email || '')
            .replace(/\{\{phone\}\}/g, phone || '')
          const emailSubject = (page.email_subject || 'Registration Confirmed').replace(/\{\{name\}\}/g, name || 'there')
          ...
          if (gmailRes.ok) {
            ...
            console.log('Confirmation email sent via Gmail (background)')
          } else {
      ...
      // is returned so the email send completes without blocking the user.
      try {
    ...
    // Also move the landing_pages count update into the background — the
    // user does not need to wait for it.
    try {
      // @ts-ignore
